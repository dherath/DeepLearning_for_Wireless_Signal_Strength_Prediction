...
#-- creating the guided training data set

guided_training_percentage = 0.3 # 30% of the total training data will be used for guided training
sz = len(X_train_data)# the total data that is for training
guided_data_size = int(round(guided_training_percentage * sz))
unguided_data_size = sz - guided_data_size

X_train_guided_data = X_train_data[0:guided_data_size]
X_train_unguided_data = X_train_data[guided_data_size:-1]

X_train_guided = np.array(X_train_guided_data)
X_train_unguided = np.array(X_train_unguided_data)

Y_train_guided_data = Y_train_data[0:guided_data_size]
Y_train_unguided_data = Y_train_data[guided_data_size:-1]

Y_train_guided = np.array(Y_train_guided_data)
Y_train_unguided = np.array(Y_train_unguided_data)
...
# Guided Training
#------------------------------------------------------
rnn_model =graph(feed_previous=False) # false for guided training
saver = tf.train.Saver()
...
with tf.Session() as sess:
    ...
    print "--- guided training started ---"
    #init.run()
    sess.run(init)
    # -- training    
    while CONTINUE_FLAG:
        #-----------------------------------
        feed_dict = {rnn_model['enc_inp'][t]:X_train_guided[:,t].reshape(-1,input_dim) for t in range(x_length)}
        feed_dict.update({rnn_model['target_seq'][t]:Y_train_guided[:,t].reshape(-1,output_dim) for t in range(y_length)})
        ...
        if ep % 100 == 0:
            ...
            temp_err = RMSE(Y_train_guided_data,temp_y_found)
        ...
        if (count >= epoch_range or ep >= EPOCH_LIMIT) and ep >= MIN_EPOCH_LIM:
            CONTINUE_FLAG = False
    print "-- guided training stopped @ epoch :",ep
    temp_saver = rnn_model['saver']()
    save_path = temp_saver.save(sess,os.path.join(save_path_name,save_object_name))
...
# Un-guided Training
#------------------------------------------------------
#rnn_model = graph(feed_previous=False)
...
with tf.Session() as sess:
    print "--- unguided training started ---"
    sess.run(init)
    ...
    while CONTINUE_FLAG:
        #-----------------------------------
        feed_dict = {rnn_model['enc_inp'][t]:X_train_unguided[:,t].reshape(-1,input_dim) for t in range(x_length)}
        feed_dict.update({rnn_model['target_seq'][t]:Y_train_unguided[:,t].reshape(-1,output_dim) for t in range(y_length)})
        train_t,loss_t,out_t = sess.run([rnn_model['train_op'],rnn_model['loss'],rnn_model['reshaped_outputs']],feed_dict)
        train_loss.append(loss_t)
        if ep % 100 == 0:
            temp_output = np.reshape(out_t,(10,-1))
            temp_output = temp_output.transpose()
            temp_y_found = temp_output.tolist()
            temp_err = RMSE(Y_train_unguided_data,temp_y_found)
            train_RMSE.append(temp_err)
            print ep," loss :",loss_t ," output size :",np.array(out_t).shape
            #-------------------- STATE LOGGER--------------------------------
            # log state of identified values every 2000 epochs
        ...
        if (count >= epoch_range or ep >= EPOCH_LIMIT) and ep >= MIN_EPOCH_LIM:
            CONTINUE_FLAG = False
    print "-- un-guided training stopped @ epoch :",ep
    ...
    name =  "Seq2seq_"+name_flag+"_LSTM_RANDOM_TRAIN_RMSE_x_"+str(x_length)+"_y_"+str(y_length)+"data.txt"
    writetofile(name,random_train_RMSE) # saving the train RMSE values for every 200th epoch
